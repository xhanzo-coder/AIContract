...
const Home: React.FC = () => {
  ...
  const scrollToBottom = (smooth = false) => {
    // 确保滚动只在MessagesArea容器内进行，不影响整体页面布局
    if (messagesEndRef.current) {
      const messagesArea = messagesEndRef.current.closest('[data-messages-area="true"]');
      if (messagesArea) {
        if (smooth) {
        ...
        <MainContent>
          ...
          {currentView === 'chat' && (
            <ChatScreen className="active main-content">
              ...
              <MessagesArea data-messages-area="true">
...
const MessagesArea = styled.div`
  ...
  @media (max-width: 768px) {
    padding: 16px;
    padding-bottom: 16px; /* 移动端也大幅减少底部内边距 */
  }
  
  @media (max-width: 480px) {
    padding: 12px;
    padding-bottom: 12px; /* 小屏设备保持一致的底部内边距 */
  }
`;
